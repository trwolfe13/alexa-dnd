...
const createType = (dir, file, name) => {
  const type = { name, values: [] };
  const list = JSON.parse(fs.readFileSync(path.join(dir, file)));
  type.values = list.map(defaultMap);
  return type;
};
...
module.exports = {
  ...
  createEquipment: dir => createType(dir, 'equipment.json', 'Equipment'),
  createBackground: dir => createType(dir, 'backgrounds.json', 'Background'),
  createFeat: dir => createType(dir, 'feats.json', 'Feat'),
  createMonsters: dir => createType(dir, 'monsters.json', 'Monster'),
  createSubclass,
  createSubrace
};